...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String shortened1 = str1.substring(0, str1.length() - overlap);
        /*
         * combination is shortened1 * str2, thus, simply use java's string
         * concatenation in the return statement.
         */
        return (shortened1 + str2);
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        /*
         * Maybe slightly unoptimized??? I think it works good, however, if you
         * guys (Huy, Steven) know another more efficient implementation just
         * comment on this.
         */
        if (temp.size() > 0) { // Check for adding a second or more element
            while (temp.size() > 0) {
                String tempStr = temp.removeAny();
                // superstring check
                if (tempStr.contains(str)) { // Check if the element contains str
                    containsStr = true;
                }
                strSet.add(tempStr); // add all elements back
            }
            if (!containsStr) { // add str when its not already in the set
                temp.transferFrom(strSet);
                while (temp.size() > 0) {
                    String tempStr = temp.removeAny();
                    if (!str.contains(tempStr)) {
                        // add elements that are not substrings of str
                        strSet.add(tempStr);
                    }
                }
                strSet.add(str);
            }
        } else { // If this is the first element we are adding then don't check
            strSet.add(str);
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        /*
         * Check pre-condition of addToSetAvoidingSubstrings by calling on an
         * initially empty set, so that anything added will always be something
         * considered CONTAINS_NO_SUBSTRING_PAIRS(linesFromInput)
         */
        Set<String> lines = new Set1L<>(); // Very descriptive variable names ik
        while (!input.atEOS()) {
            // Get a new line read for each line in the file.
            String line = input.nextLine();

            /*
             * If we are at the first line in the file lines will be empty and
             * line will always be added to lines.
             */
            addToSetAvoidingSubstrings(lines, line);
        }
        return lines;
    }
    ...
    public static void assemble(Set<String> strSet) {
        ...
        boolean done = false;
        /*
         * Pre-condition for bestOverlap bestTwo.length >= 2 is checked here. In
         * addition, CONTAINS_NO_SUBSTRING PAIRS is initially checked in
         * linesFromInput and thus no longer needs to be checked.
         */
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        /*
         * Keep original string with separators and make a new one that can be
         * changed.
         */
        String textCopy = text.substring(0, text.length());
        // While there still exists a '~' character in the text.
        while (textCopy.indexOf('~') != -1) {
            out.println(textCopy.substring(0, textCopy.indexOf('~')));
            textCopy = textCopy.substring(textCopy.indexOf('~') + 1);
        }
    }
