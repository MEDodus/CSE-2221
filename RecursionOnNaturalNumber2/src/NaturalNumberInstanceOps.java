...
public final class NaturalNumberInstanceOps extends NaturalNumber2 {
    ...
    public void subtract(NaturalNumber n) {
        ...
        int thisHighDigit = this.divideBy10();
        int nHighDigit = n.divideBy10();

        if (!n.isZero()) {
            this.subtract(n);
        }
        thisHighDigit -= nHighDigit;
        if (thisHighDigit < 0) {
            thisHighDigit += RADIX;
            this.decrement();
        }
        this.multiplyBy10(thisHighDigit);
        n.multiplyBy10(nHighDigit);
    }

    @Override
    public void power(int p) {
        assert p >= 0 : "Violation of: p >= 0";
        NaturalNumber digit = new NaturalNumber2(this);
        if (p == 0) {
            this.setFromInt(1);
        } else {
            if (p % 2 == 0) {
                this.power(p / 2);
                this.multiply(new NaturalNumber2(this));
            } else {
                this.multiply(digit);
                this.power((p - 1) / 2);
                this.multiply(digit);
