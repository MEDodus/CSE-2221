...
public final class XMLTreeNNExpressionEvaluator2 {
    ...
    private static NaturalNumber evaluate(XMLTree exp) {
        ...
        // If we are not at the bottom of the tree keep calling evaluate
        // on the children
        if (numChild > zero) {
        ...
        // only if the string can be set from the NaturalNumber
        if (exp.label().equals(number)) { // <number> node
            ...
            // If not then check what node we are at and evaluate based
            // of of both children

            if (exp.label().equals(times)) { // <times> node
                leftResult.multiply(rightResult);

            } else if (exp.label().equals(divide)) { // <divide> node
            ...
            } else if (exp.label().equals(plus)) { // <plus> node
