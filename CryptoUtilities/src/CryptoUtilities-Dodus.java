...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        // Recurse until m is 0
        if (!m.isZero()) {
            NaturalNumber remainder = n.divide(m);
            n.transferFrom(remainder);
            reduceToGCD(m, n);
        } else {
            // Due to references switching on each recursive call, m references n.
            m.transferFrom(n);
        }
    }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber remainder = copy.divide(new NaturalNumber2(2));

        if (remainder.compareTo(new NaturalNumber2()) == 0) {
            even = true;
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        nCopy.decrement(); // This will be (n-1)
        final NaturalNumber sqr = new NaturalNumber2(2);
        boolean isComposite = false;

        // Square w and mod by n
        powerMod(w, sqr, n);
        if (w.compareTo(one) == 0) {
            isComposite = true;
        }

        // Restore w from wCopy in this case copy
        w.copyFrom(wCopy);
        // w ^ (n-1) mod n method call
        powerMod(w, nCopy, n);
        if (w.compareTo(one) != 0) {
            isComposite = true;
        }

        // Restore w from wCopy in this case transfer
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // I'm going to use 25 candidates for my implementation
        int candidateCnt = 0;
        final int total = 25;

        // Boolean to return primality results of n
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            /*
             * Generate 25 random candidates in [2, n-1] only for n >= 5.
             */
            boolean isComposite = false;
            while (candidateCnt < total && !isComposite) {
                NaturalNumber rand = new NaturalNumber2();
                /*
                 * Check if rand is 1 or 0, both fail the requires clause as
                 * well as if it is less than n - 1.
                 */
                ...
                while (!checkRand) {
                    ...
                    checkRand = true;
                    if (rand.compareTo(new NaturalNumber2(1)) <= 0) {
                        checkRand = false;
                    }
                }
                isComposite = isWitnessToCompositeness(rand, n);
                candidateCnt++;
            }
            isPrime = !isComposite;
        }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        /*
         * Move through only odd numbers because primality only occurs in
         * subsets of odd numbers
         */
        final NaturalNumber two = new NaturalNumber2(2);
        do {
            if (isEven(n)) { // If even proceed to next odd number.
                n.increment();
            } else { // If odd proceed to next odd number.
                n.add(two);
            }
        } while (!isPrime2(n));
