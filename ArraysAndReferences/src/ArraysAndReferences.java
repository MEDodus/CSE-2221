...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber productOfArrayElements(
        ...
        final int zero = 0;
        final int multiplyConstant = 1;
        final int length = nnArray.length;
        NaturalNumber product = new NaturalNumber1L(multiplyConstant);
        product.setFromInt(multiplyConstant);

        for (int idx = zero; idx < length; idx++) {
            product.multiply(nnArray[idx]);
